...
export default function ProfilePage() {
  ...
  const [streak, setStreak] = useState<number>(auth?.user?.day_streak ?? 0);
  const [totalXp, setTotalXp] = useState<number>(auth?.user?.total_xp ?? 0);
  const [league, setLeague] = useState<string>(auth?.user?.current_league ?? 'Bronze');
  const [topFinishes, setTopFinishes] = useState<number>(auth?.user?.top3_finishes ?? 0);

  useEffect(() => {
    if (auth?.user) {
      setTotalXp(auth.user.total_xp ?? 0);
      setStreak(auth.user.day_streak ?? 0);
      setLeague(auth.user.current_league ?? 'Bronze');
      setTopFinishes(auth.user.top3_finishes ?? 0);
    }
  }, [auth?.user]);
          ...
          <div className="flex-1 space-y-8">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Link href={route('achievements.index')} className="text-xs font-bold text-purple-600 hover:text-purple-800 transition-colors">
                  VIEW ALL
                </Link>
              </div>
